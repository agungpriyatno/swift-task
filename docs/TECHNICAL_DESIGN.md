...
Dokumen ini merinci arsitektur teknis untuk **SwiftTask**, aplikasi manajemen tugas minimalis. Arsitektur ini mengutamakan performa tinggi menggunakan runtime **Bun**, keamanan isolasi data pengguna, dan pengalaman pengguna yang responsif melalui integrasi *Server Components* dan *Server Actions* pada Next.js.
...
| **Backend Framework** | NestJS | Menyediakan core business logic dan manajemen dependensi yang terstruktur. |
| **Frontend Framework** | Next.js (App Router) | Pendekatan *Server Component First* untuk efisiensi loading data. |
...
Desain skema difokuskan pada relasi satu-ke-banyak antara User, Kategori, dan Tugas, dengan mekanisme *cascading* untuk keamanan data.
...
* **Performance**: Memanfaatkan *Streaming* dengan Suspense untuk menampilkan kerangka (skeleton) Shadcn saat data sedang dimuat.
...
* **Category Deletion**: Sesuai US-03, logic di NestJS/Server Action akan melakukan *batch update* untuk mengubah categoryId tugas yang terdampak menjadi ID kategori "Uncategorized" sebelum menghapus kategori target.
